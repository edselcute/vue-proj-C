        ...
        methods: {
            ...
            async submit() {
                ...
                const confirmed = await vm
                    .$swal({
                        title: 'Are you sure?',
                        text: `Deposit ₱${vm.cash_payload.amount}`,
                        type: 'warning',
                ...
                if (confirmed) {
                    const res = await vm.transaction_dp({ id: vm.user_id, amount: vm.formatStringNumber(vm.cash_payload.amount), status: 0 });
                    if (res.success) {
            ...
            getDeposits() {
                this.transaction_gl({ page_no: 1, type: 1, user_id: this.user_id });
            }
        },
        mounted() {
            var vm = this;
